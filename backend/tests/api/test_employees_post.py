...
def test_post_employee_returns_201_with_created_resource(api_client):
    response = api_client.post("/employees", json=_payload())
...
def test_post_duplicate_email_returns_409(api_client):
    ...
    api_client.post(
        "/employees",
        json=_payload(employee_id="EMP-A", email="dup@example.com"),
    )

    response = api_client.post(
        "/employees",
        json=_payload(employee_id="EMP-B", email="dup@example.com"),
    )
...
def test_post_with_invalid_email_returns_422(api_client):
    ...
    is caught."""
    response = api_client.post("/employees", json=_payload(email="not-an-email"))
...
def test_post_with_negative_salary_returns_422(api_client):
    ...
    response = api_client.post("/employees", json=_payload(salary="-1.00"))
